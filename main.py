...
def check_pdf_content(pdf_path):
    ...
    #Definindo-se a linha de corte mínima para classificar o PDF como com presença de TABELA textual;
    min_text_length = 50

    try:
        extracted_text = ""

        #Buscando-se presença de texto
        ...
        #Buscando-se presença de imagem no PDF
    ...
    if has_text and has_images:   #presença de tabela em texto + imagem com necessidade de OCR
        return "MIX"
    elif has_text:
        return "TABELA"
    elif has_images:
...
def identify_pdf_type(pdf_path):
    ...
    #Primeiro, verificará se existe texto ou imagens diretamente no documento retornando qual o tipo do PDF
    ...
    # Caso a função "check_pdf_content" nao identificar o tipo uma IA realizará uma análise mais direta
...
def process_pdf(pdf_path):
    ...
    # Identificação Automática do Tipo de PDF**
    doc_type = identify_pdf_type(pdf_path)

    # Supervisor pode sobrescrever o tipo, se necessário**
    logging.info(f"⚙️ Supervisor pode alterar a escolha automática do worker.")

    # Escolhe o worker correto com base no tipo identificado**
    extracted_data = None
    ...
    if doc_type == "TABELA":
        ...
        logging.error(f"Erro: Tipo de PDF desconhecido ({pdf_path})")
        return

    # Verifica se houve extração válida**
    if not extracted_data:
        logging.warning(f"Nenhum dado extraído do PDF {pdf_path}. Pulando para o próximo arquivo.")
        return
    ...
    # Criação ou atualização do CSV Consolidado**
